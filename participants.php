...
require_once('../mooin1pager/lib.php');
...
$PAGE->set_url('/course/format/mooin1pager/participants.php', array(
		'id' => $courseid,
        'page' => $page,
        'perpage' => $perpage,
        'mode' => $mode,
...
$PAGE->set_url('/course/format/mooin1pager/participants.php', array('id' => $course->id));
...
$rolenamesurl = new moodle_url("$CFG->wwwroot/course/format/mooin1pager/participants.php?contextid=$context->id&sifirst=&silast=");

$rolenames = role_fix_names(get_profile_roles($context), $context, ROLENAME_ALIAS, true);
if ($isfrontpage) {
    $rolenames[0] = get_string('allsiteusers', 'role');
} else {
    $rolenames[0] = get_string('allparticipants', 'format_mooin1pager');
}
...
echo html_writer::start_div('mooin1pager-subpage-bg');
echo '<div class="userlist">';
//echo '<div class="mooin1pager-md-container">';

//echo navbar('participants');
echo '<h2>'.get_string("participants","format_mooin1pager").'</h2>';
...
$baseurl = new moodle_url('/course/format/mooin1pager/participants.php', array(
...
if ($mycourses = enrol_get_my_courses()) {
    $courselist = array();
    $popupurl = new moodle_url('/course/format/mooin1pager/participants.php?roleid='.$roleid.'&sifirst=&silast=');
...
// course/format/mooin1pager
if ($mode === MODE_BRIEF && !isset($hiddenfields['city']) && has_capability('format/mooin1pager:readuserpage', $context)) { // oncampus sprint
    $tablecolumns[] = 'city';
    $tableheaders[] = get_string('city');
}
if ($mode === MODE_BRIEF && !isset($hiddenfields['country']) && has_capability('format/mooin1pager:readuserpage', $context)) { // oncampus sprint
...
// added by oncampus
// oncampus sprint
if (has_capability('format/mooin1pager:readuserpage', $context)) {
	$tablecolumns[] = 'badges';
...
foreach ($userlist as $lu) {
    ...
    $map_title = get_string('map_title', 'format_mooin1pager');
        $map_descr = get_string('map_descr', 'format_mooin1pager');
        ...
        echo $OUTPUT->render_from_template('format_mooin1pager/map_manage', $templatecontext);
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
echo('<br>');
...
if ($roleid > 0) {
    ...
    if ($course->id == SITEID and $roleid < 0) {
        ...
        $strallparticipants = get_string('allparticipants', 'format_mooin1pager');
    }
...
if ($mode === MODE_USERDETAILS) {  // Print simple listing.
	...
	if ($totalcount > $perpage) {
		...
		echo '&nbsp;'.html_writer::link(new moodle_url('/course/format/mooin1pager/participants.php', array('id' => $course->id, 'tab' => 1)), get_string('cancel'));
		echo '</div></form>'."<br />";
	}
    ...
    if ($userlist) {
        ...
        foreach ($userlist as $user) {
            ...
            // course/format/mooin1pager
            if ($mode === MODE_BRIEF && !isset($hiddenfields['city']) && has_capability('format/mooin1pager:readuserpage', $context)) { // oncampus sprint && has_capability('course/mooin1pager:readuserpage', $context)
                $data[] = $user->city;
            }
            // course/format/mooin1pager
            if ($mode === MODE_BRIEF && !isset($hiddenfields['country']) && has_capability('format/mooin1pager:readuserpage', $context)) { // oncampus sprint && has_capability('course/mooin1pager:readuserpage', $context)
                $data[] = $country;
            }
            // course/format/mooin1pager
            if (!isset($hiddenfields['lastaccess']) && has_capability('format/mooin1pager:readuserpage', $context)) { // oncampus sprint  && has_capability('course/mooin1pager:readuserpage', $context)
                $data[] = $lastaccess;
            }
            // course/format/mooin1pager
			/* if (has_capability('format/mooin1pager:readuserpage', $context)) {
            ...
            if ($mode === MODE_BRIEF && !isset($hiddenfields['badges']) && has_capability('format/mooin1pager:aluhatsoff', $context)) {
...
$out = html_writer::tag('div', get_string('awarded_badges', 'format_mooin1pager'), array('class' => 'oc_badges_text'));
echo html_writer::tag('h2', $out);
// echo html_writer::tag('div', get_string('lastday', 'format_mooin1pager'), array('class' => 'oc_badges_text'));
// display_badges(0, $courseid, 24 * 60 * 60);
//echo html_writer::tag('div', get_string('lastweek', 'format_mooin1pager'), array('class' => 'oc_badges_text'));
ob_start();
...
if ($out != '') {
    ...
    echo html_writer::tag('div', get_string('no_badges_awarded', 'format_mooin1pager'), array('class' => 'oc-no-badges'));
}
...
// 		echo html_writer::tag('div', html_writer::link($unenrolurl, get_string('unenrol', 'format_mooin1pager') )); // , array('class' => 'oc-kurs-abmeldung'
