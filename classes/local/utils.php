...
namespace format_mooin1pager\local;
...
class utils {
    ...
    public static function complete_section($section) {
        global $USER;
        set_user_preference('format_mooin1pager_section_completed_' . $section, 1, $USER->id);
    }

    public static function is_course_completed($course_id) {
        global $DB;
        $is_course_completed = false;
        if ($course_chapters = $DB->get_records('format_mooin1pager_chapter', array('courseid' => $course_id))) {
    ...
    public static function setgrade($contextid, $score, $maxscore) {
        ...
        if ($score >= $user_grade) {
            ...
            /* <script>;
                var divId = String('mooin1pagerection' + $_POST['sectionid']); // mooin1pagerection-progress
                var textDivId = String('mooin1pagerection-text-' + $_POST['sectionid']); // mooin1pagerection-progress-text-
    ...
    public static function get_hvp_section_progress($courseid, $sectionid, $userid) {
        ...
        if ($activities == 0) {
            if (get_user_preferences('format_mooin1pager_section_completed_' . $sectionid, 0, $userid) == 1) {
                return 100;
            } else {
                return 0;
            }
    ...
    public static function set_user_coordinates($userid, $lat, $lng) {
        set_user_preference('format_mooin1pager_user_coordinates', $lat . '|' . $lng, $userid);
    }
    ...
    public static function get_user_in_course($courseid) {
        ...
        if (isset($enrol_data)) {
            ...
            $participants_url = new moodle_url('/course/format/mooin1pager/participants.php', array('id' => $courseid));
            $participants_link = html_writer::link($participants_url, get_string('show_all_infos', 'format_mooin1pager'), array('title' => get_string('participants', 'format_mooin1pager')));
        } else {
            $out .= html_writer::div(get_string('no_user', 'format_mooin1pager'), 'no_user_class');
        }
    ...
    public static function get_last_news($courseid, $forum_type) {
        ...
        if (!empty($latestpost = $DB->get_record_sql($sql, $params))) {
            ...
            if ($forum_type == 'news') {
                ...
                //     $new_news .= html_writer::link($newsurl, get_string('unread_news_single', 'format_mooin1pager') . get_string('all_news', 'format_mooin1pager'), array('title' => get_string('all_news', 'format_mooin1pager'), 'class' => 'primary-link'));
                ...
                //     $new_news .= html_writer::link($newsurl, get_string('unread_news', 'format_mooin1pager') . get_string('all_news', 'format_mooin1pager'), array('title' => get_string('all_news', 'format_mooin1pager'), 'class' => 'primary-link'));
                // } else {
                //     $new_news = false;
                // }
            }
    ...
    public static function set_chapter($sectionid) {
        ...
        if ($DB->get_record('format_mooin1pager_chapter', array('sectionid' => $sectionid))) {
            return;
        }
        ...
        if (!$csectiontitle) {
            $csectiontitle = get_string('new_chapter', 'format_mooin1pager');
        }
        ...
        $chapter->chapter = 0;
        $DB->insert_record('format_mooin1pager_chapter', $chapter);
    ...
    public static function unset_chapter($sectionid) {
        ...
        $DB->delete_records('format_mooin1pager_chapter', array('sectionid' => $sectionid));
        if ($csection = $DB->get_record('course_sections', array('id' => $sectionid))) {
            self::sort_course_chapters($csection->course);
        }
    }
    ...
    public static function sort_course_chapters($courseid) {
        ...
        foreach ($coursechapters as $coursechapter) {
            $number++;
            if ($existingcoursechapter = $DB->get_record('format_mooin1pager_chapter', array('id' => $coursechapter->id))) {
                $existingcoursechapter->chapter = $number;
                $DB->update_record('format_mooin1pager_chapter', $existingcoursechapter);
            }
        }
    }
    ...
    public static function get_sectionids_for_chapter($chapterid) {
        ...
        if ($chapter = $DB->get_record('format_mooin1pager_chapter', array('id' => $chapterid))) {
            ...
            if ($chaptersection = $DB->get_record('course_sections', array('id' => $chapter->sectionid))) {
                if ($nextchapters = $DB->get_records('format_mooin1pager_chapter', array('courseid' => $chapter->courseid, 'chapter' => $chapter->chapter + 1))) {
                    // there is a bug somewhere - the mooin1pager_chapter table is not updated correctly
    ...
    public static function get_course_chapters($courseid) {
        ...
        $sql = 'SELECT c.*, s.section
                  FROM {format_mooin1pager_chapter} as c, {course_sections} as s
    ...
    public static function course_navbar() {
        ...
        return $OUTPUT->render_from_template('format_mooin1pager/custom_navbar', $templatecontext);
    }
...
*/
    ...
    public static function get_chapter_info($chapter) {
        ...
        foreach ($sectionids as $sectionid) {
            ...
            $last_section = get_user_preferences('format_mooin1pager_last_section_in_course_' . $chapter->courseid, 0, $USER->id);
    ...
    public static function is_course_started($course) {
        ...
        $last_section = get_user_preferences('format_mooin1pager_last_section_in_course_' . $course->id, 0, $USER->id);
        if ($last_section) {
            return true;
        } else {
            return false;
    ...
    static function get_headerimage_url($courseid, $mobile = true) {
        ...
        $url = new moodle_url('/pluginfile.php/' . $context->id . '/format_mooin1pager/' . $filearea . '/' . $courseid . '/0/' . $filename);
        return $url;
    }
    ...
    public static function print_badges_html($records, $details = false, $highlight = false, $badgename = false) {
        ...
        usort($records, function ($first, $second) {
            ...
            $f = get_user_preferences('format_mooin1pager_new_badge_' . $first->issuedid, 0, $USER->id);
            $s = get_user_preferences('format_mooin1pager_new_badge_' . $second->issuedid, 0, $USER->id);
            if ($f < $s) {
                return 1;
            }
            if ($f == $s) {
        ...
        foreach ($records as $key => $record) {
            ...
            if (isset($record->uniquehash)) {
                ...
                $badgeisnew = get_user_preferences('format_mooin1pager_new_badge_' . $record->issuedid, 0, $USER->id);
            } else {
            ...
            if (is_string($templ) != 1) {
                ...
                for ($i = 0; $i < count($templ); $i++) {
                    if ($templ[$i]->url != '#') { // if certificate is issued to user
                        ...
                        if (get_user_preferences('format_mooin1pager_new_certificate_' . $certmod . '_' . $issuedid, 0, $USER->id) == 1) {
    ...
    public static function set_new_certificate($awardedtoid, $issuedid, $modulename) {
        set_user_preference('format_mooin1pager_new_certificate_' . $modulename . '_' . $issuedid, true, $awardedtoid);
    }
    ...
    public static function unset_new_certificate($viewedbyuserid, $issuedid, $modulename) {
        ...
        if ($record = $DB->get_record_sql($sql, $params)) {
            if ($record->userid == $viewedbyuserid) {
                unset_user_preference('format_mooin1pager_new_certificate_' . $modulename . '_' . $record->id, $viewedbyuserid);
            }
        }
    }
    ...
    static function get_user_coordinates($user) {
        if ($user->city != '') {
            ...
            $url = get_config('format_mooin1pager', 'geonamesapi_url');
            $apiusername = get_config('format_mooin1pager', 'geonamesapi_username');
    ...
    public static function set_new_badge($awardedtoid, $badgeissuedid) {
        set_user_preference('format_mooin1pager_new_badge_' . $badgeissuedid, true, $awardedtoid);
    }
        ...
        if ($records = $DB->get_records_sql($sql, $params)) {
            if (count($records) == 1) {
                if ($records[array_key_first($records)]->userid == $viewedbyuserid) {
                    unset_user_preference('format_mooin1pager_new_badge_' . $records[array_key_first($records)]->id, $viewedbyuserid);
                }
            }
        }
    }
